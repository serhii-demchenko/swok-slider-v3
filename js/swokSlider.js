export default class {
	...
	setSlidersWidth() {
		for (let i = 0; i < this.slider.children.length; i += 1) {
			if (this.slider.children[i].classList.contains("swok-slider__wrapper")) {
				this.sliderWidth = this.slider.children[i].style.width = this.slider.clientWidth * this.sliderCount + "px";
				break;
			}
		}
	}
	calculateSlideCoordinates() {
		...
		for (let i = 0; i < this.sliderCount; i += 1) {
			this.sliderCoordinateArray.push(0 - (Number.parseInt(this.sliderWidth) / this.sliderCount) * i);
		}
	}
	...
	prevSlideHandler() {
		this.curentSlide = this.curentSlide - 1 > 0 ? this.curentSlide - 1 : this.sliderCount - 1;
		this.changeSlide(true);
	}
	nextSlideHandler() {
		this.curentSlide += this.curentSlide + 1 === this.sliderCount ? -this.sliderCount + 1 : 1;
		this.changeSlide(true);
	}
	resizeHandler() {
